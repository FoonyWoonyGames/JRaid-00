...
public class AbilityHack extends Ability {

	public AbilityHack(Player user) {
		super(user);
		...
		if(isCooled()) {
			...
			for(int i = 0; i < user.getEntitiesInFront(0, 0, 0).size(); i++) {
				...
				if(e.type == user.getEnemy()) {
					enemyToHit = e;
					enemyToHit.hurt(user, 15, Math.random());
					enemyToHit.push(delta, user.getDirection(), 125);
					enemyToHit.addStatusEffect(new EffectStunned(user, enemyToHit, 1.0f, 1, 3000));
					user.getWorld().spawnParticle(Particle.PARTICLE_BLOOD, 5, 25, 16, 16, enemyToHit.getX(), enemyToHit.getY(), false, 5, 0, 1, 360, false);
				}
			}
		}
	}
