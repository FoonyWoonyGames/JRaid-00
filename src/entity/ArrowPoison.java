...
import effect.EffectPoisoned;
import render.Animation;
import render.Camera;
import util.Window;
import world.World;

public class ArrowPoison extends Projectile {
	...
	public ArrowPoison(Transform transform, Entity sender) {
	...
	long particleSpawned = 0;

	@Override
	public void update(float delta, Window win, Camera camera, World world) {
		Vector2f movement = new Vector2f();
		
		if(System.currentTimeMillis() - particleSpawned > 10) {
			particleSpawned = System.currentTimeMillis();
			world.spawnParticle(Particle.PARTICLE_POISON, 3, 18, 16, 16, getX(), getY(), false, 2, 0, 2, 45, false);
		}
		if(!hasHit) {
			for(int i = 0; i < world.getEntityList().size(); i++) {
				...
				if(e.type == Entity.TYPE_ENEMY && isColliding(e)) {
					hasHit = true;
					e.hurt(sender, 4, Math.random());
					e.addStatusEffect(new EffectPoisoned(sender, e, 1.0f, 500, 3000));
				}
			}
		}
